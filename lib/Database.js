...
var Database = module.exports = Class.extend({
  ...
  getOrderBy: function (model, orders) {
    ...
    for (var i = 0; i < pairs.length; i++) {
      ...
      var field = model.fields[pair[0]];
      if (field) {
        var direction = (pair[1] || '').replace(/^.*?(asc|desc)?.*?$/,'$1');
        orders.push(field.as + ' ' + direction);
      }
    }
    ...
    for (var fieldName in filters) {
      ...
      if (field) {
        ...
        if (value instanceof Array) {
          ...
          else if (operator === 'IN' && value[1].length) {
