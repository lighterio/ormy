...
var Database = module.exports = Class.extend({
  ...
  breakStringIntoArrays: function (orders) {
    var arr = orders.split(',');
    var set = [];
    for (var i in arr) {
      set.push(arr[i].trim().split(' '));
    }
    return set;
  },

  /**
   * Builds the order by query segment
   */
  getOrderBy: function (model, orders) {
    var db = this;
    if (typeof orders == 'string' || orders instanceof String) return db.getOrderBy(model, db.breakStringIntoArrays(orders));
    if (!orders || (orders && orders.length === 0)) return '';

    var query = ' ORDER BY';
    for (var i in orders) {
      var order = orders[i];
      var direction = (order[1] || '').replace(/^.*?(asc|desc)?.*?$/,'$1');
      var field = model.fields[order[0].trim()];
      if(field) {
        query += ' ' + db.sanitize(field.as) + ' ' + direction + ((orders.length - 1) != i ? ',': '');
      }
    }

    // query failed to build, return empty
    if (query === ' ORDER BY') {
      return '';
    ...
    for (var fieldName in item) {
      ...
      if (field && (fieldName !== exclude)) {
        ...
        if (item[fieldName] === null) {
          ...
          sets.push(db.sanitize(field.column) + "='" + value + "'");
        }
      }
    }
    if (model.createdField && (mode === 'create')) {
      sets.push(db.sanitize(model.createdField.name) + "=NOW()");
    }
    if (model.modifiedField) {
      sets.push(db.sanitize(model.modifiedField.name) + "=NOW()");
    }
  ...
  /**
   * Sanatize method to escape & prevent sql injections
   */
  sanitize: function sanitize(field) {
  ...
  /**
   * Builds the where query segment
   */
  getWhereSql: function getWhereSql(model, filters, where) {
    ...
    for (var fieldName in filters) {
      var field = model.fields[fieldName];
      if (field) {
        var column = db.sanitize(field.column);
